...
export default function AdminPage() {
  ...
  const [securityHealthCheck, setSecurityHealthCheck] = useState<any>(null);
  const [lastSecurityCheck, setLastSecurityCheck] = useState<number>(0);

  // Enhanced admin verification with security logging
  const isUserAdmin = (() => {
    const walletAddress = connected ? publicKey?.toString() : algorandAddress;
    const network = connected ? 'solana' : 'algorand';
    
    if (!walletAddress) return false;
    
    const authorized = isAuthorizedAdmin(walletAddress, network);
    ...
    if (walletAddress) {
      logAdminAction(
        ...
        walletAddress, 
        authorized, 
        `Authorization check for ${network} wallet`
      );
    }
  ...
  useEffect(() => {
    if (mounted && isUserAdmin) {
      ...
      if (walletAddress) {
        // Create or validate admin session
        const session = createAdminSession(walletAddress, network);
        setAdminSession(session);
        
        // Load admin data with security verification
        loadPricingData();
  ...
  }, [mounted, isUserAdmin, connected, publicKey, algorandAddress]);
  ...
  const performSecurityCheck = () => {
    ...
    if (healthCheck.status === 'critical') {
      ...
      console.warn('[ADMIN_SECURITY] Security warnings:', healthCheck.issues);
    }
  };

  // Enhanced rate-limited admin action wrapper
  const executeAdminAction = async (
    ...
    try {
      ...
      // For now, we'll use a simple verification (in production, this would require wallet signature)
      const verification = await verifyAdminRequest(walletAddress, challenge, 'auto-verified', action);
      ...
      logAdminAction(action, walletAddress, true, actionDescription);
      
      return result;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      logAdminAction(action, walletAddress, false, `${actionDescription} failed: ${errorMessage}`);
      throw error;
    }
  };
  ...
  // Enhanced save platform settings with security
  const savePlatformSettings = async () => {
    await executeAdminAction(
      AdminActionType.PLATFORM_SETTINGS,
      async () => {
        ...
        try {
          ...
          const result = await updateAdminConfig({
            fees: {
              algorandMainnetFee: 200000,
              algorandTestnetFee: 0,
              solanaMainnetFee: 50000,
              solanaDevnetFee: 0,
            }
          });
  ...
  // Enhanced save Algorand fees with security
  const saveAlgorandFees = async () => {
    await executeAdminAction(
      AdminActionType.UPDATE_FEES,
      async () => {
        ...
        try {
          ...
          if (result.success) {
            ...
            toast({
              title: "✅ Fees Updated",
              description: "Algorand fees updated successfully",
              duration: 5000,
            });
          } else {
      ...
      'Algorand fee configuration update'
    );
  };

  // Enhanced save pricing with security
  const handleSavePricing = async (network: string) => {
    await executeAdminAction(
      AdminActionType.UPDATE_PRICING,
      async () => {
      ...
      `Pricing configuration update for ${network}`
    );
  };

  // Enhanced platform initialization with security
  const handleInitialize = async () => {
    await executeAdminAction(
      AdminActionType.INITIALIZE_PLATFORM,
      async () => {
        ...
        if (feeInLamports < 0 || feeInLamports > 1000000000000) {
          throw new Error('Creation fee must be between 0 and 1000 SOL');
        }

        if (publicKey && !publicKey.equals(ADMIN_WALLET)) {
          throw new Error('Admin access required. Only the designated admin wallet can initialize the platform.');
        }
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="min-h-screen p-6 relative z-10">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          {/* Enhanced Wallet Status with Security Info */}
          <div className="snarbles-card-premium p-8 snarbles-glow-green">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {securityHealthCheck && (
                      <div className={`px-2 py-1 rounded text-xs font-medium ${
                        securityHealthCheck.status === 'good' ? 'bg-green-500/20 text-green-400' :
                        securityHealthCheck.status === 'warning' ? 'bg-yellow-500/20 text-yellow-400' :
                        'bg-red-500/20 text-red-400'
                      }`}>
                        Security: {securityHealthCheck.status}
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="snarbles-glass-subtle p-6 rounded-xl">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="snarbles-body text-sm mb-2">Connected Wallet ({connected ? 'Solana' : 'Algorand'}):</p>
                  <code className="snarbles-glass-subtle p-3 rounded text-sm font-mono block break-all border border-green-500/30">
                    {connected ? publicKey?.toString() : algorandAddress}
                  </code>
                </div>
                
                <div>
                  <p className="snarbles-body text-sm mb-2">Admin Wallet (Expected):</p>
                  <code className="snarbles-glass-subtle p-3 rounded text-sm font-mono block break-all border border-blue-500/30">
                    {connected ? ADMIN_CONFIG.solana.wallet : ADMIN_CONFIG.algorand.wallet}
                  </code>
                </div>
              </div>
              ...
              {adminSession && (
                <div className="mt-4 pt-4 border-t border-border/50">
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-4">
                      <span className="snarbles-body">Session: Active</span>
                      <span className="snarbles-body">Actions: {adminSession.actionCount}</span>
                      <span className="snarbles-body">
                        Login: {new Date(adminSession.loginTime).toLocaleTimeString()}
                      </span>
                    </div>
                    {lastSecurityCheck > 0 && (
                      <span className="text-xs text-gray-400">
                        Last security check: {new Date(lastSecurityCheck).toLocaleTimeString()}
                      </span>
                    )}
                  </div>
                </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="control-center" className="space-y-8 mt-8">
