...
function TokenomicsPage() {
  ...
  const generatePDFReport = async () => {
    ...
    try {
      ...
      // Enhanced helper functions for better PDF formatting
      const addNewPageIfNeeded = (currentY: number, spaceNeeded: number) => {
        if (currentY + spaceNeeded > pageHeight - 25) {
          doc.addPage();
          return 25; // Start position on new page with top margin
        }
        return currentY;
      };
      ...
      const addStyledHeader = (title: string, subtitle: string, yPos: number) => {
        ...
        doc.text(subtitle, pageWidth / 2, yPos + 15, { align: 'center' });
        
        return yPos + 30;
      };

      // Add a styled card/section
      const addCard = (title: string, yPos: number, height: number = 40) => {
        // Card background
        ...
        return yPos + 15; // Return position for content start
      };
      ...
      yPos = addNewPageIfNeeded(yPos, 60);
      const overviewCardY = addCard('PROJECT OVERVIEW', yPos, 50);
      
      doc.setFontSize(11);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(0, 0, 0);
      doc.text(`Project Name: ${projectName || 'Unnamed Token Project'}`, margin + 10, overviewCardY + 5);
      doc.text(`Total Supply: ${totalSupply.toLocaleString()} tokens`, margin + 10, overviewCardY + 15);
      doc.text(`Analysis Date: ${new Date().toLocaleDateString()}`, margin + 10, overviewCardY + 25);
      ...
      doc.roundedRect(margin + 120, overviewCardY, 40, 12, 3, 3, 'F');
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.text(`${healthAnalysis.score}/100`, margin + 140, overviewCardY + 8, { align: 'center' });
      
      yPos += 60;
      
      // Executive Summary
      yPos = addNewPageIfNeeded(yPos, 60);
      yPos = addCard('EXECUTIVE SUMMARY', yPos, 60);
      
      const executiveSummary = `This comprehensive analysis evaluates ${projectName || 'your token project'}'s economic structure. 
With ${totalSupply.toLocaleString()} total tokens, the project shows ${riskAssessment.level.toLowerCase()} risk. 
Key highlights: ${Math.round(communityRatio * 100)}% community allocation, ${Math.round(liquidityRatio * 100)}% liquidity, 
${Math.round(teamRatio * 100)}% team allocation.`;
      
      const summaryLines = wrapText(executiveSummary, contentWidth - 20, 10);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(0, 0, 0);
      
      summaryLines.forEach((line, index) => {
        doc.text(line, margin + 10, yPos + 10 + (index * 6));
      });
      
      yPos += 70;
      
      // PAGE 2: TOKEN DISTRIBUTION
      doc.addPage();
      yPos = addStyledHeader('TOKEN DISTRIBUTION BREAKDOWN', 'Detailed allocation analysis and recommendations', 20);
      
      // Distribution Table
      yPos = addCard('ALLOCATION DETAILS', yPos, 100);
      
      // Table headers
      doc.setFillColor(241, 245, 249);
      doc.rect(margin + 5, yPos, contentWidth - 10, 12, 'F');
      doc.setDrawColor(203, 213, 225);
      doc.rect(margin + 5, yPos, contentWidth - 10, 12);
      
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(30, 41, 59);
      doc.text('CATEGORY', margin + 10, yPos + 8);
      doc.text('ALLOCATION', margin + 50, yPos + 8);
      doc.text('TOKENS', margin + 80, yPos + 8);
      doc.text('PURPOSE', margin + 120, yPos + 8);
      ...
      const distributionData = Object.entries(distribution).map(([key, data]) => ({
        ...
        purpose: key === 'team' ? 'Development & Operations' :
                key === 'investors' ? 'Private Funding' :
                key === 'community' ? 'Rewards & Incentives' :
                key === 'liquidity' ? 'DEX Liquidity' :
      ...
      distributionData.forEach((row, index) => {
        if (index % 2 === 0) {
          ...
          doc.rect(margin + 5, yPos, contentWidth - 10, 10, 'F');
        }
        doc.setDrawColor(203, 213, 225);
        doc.rect(margin + 5, yPos, contentWidth - 10, 10);
        
        doc.setFontSize(8);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(0, 0, 0);
        doc.text(row.category, margin + 10, yPos + 7);
        doc.text(`${row.percentage}%`, margin + 50, yPos + 7);
        doc.text(row.tokens.toLocaleString(), margin + 80, yPos + 7);
        doc.text(row.purpose, margin + 120, yPos + 7);
      ...
      doc.addPage();
      yPos = addStyledHeader('HEALTH ANALYSIS & RECOMMENDATIONS', 'Expert insights and strategic guidance', 20);
      
      // Strengths
      if (healthAnalysis.strengths.length > 0) {
        yPos = addCard('STRENGTHS', yPos, Math.max(30, healthAnalysis.strengths.length * 8 + 15));
        
        healthAnalysis.strengths.forEach((strength, index) => {
          const lines = wrapText(`✓ ${strength}`, contentWidth - 20, 9);
          doc.setFontSize(9);
          doc.setFont('helvetica', 'normal');
          doc.setTextColor(34, 197, 94);
          lines.forEach((line, lineIndex) => {
            doc.text(line, margin + 10, yPos + 5 + (index * 8) + (lineIndex * 4));
          });
        });
        
        yPos += Math.max(35, healthAnalysis.strengths.length * 8 + 20);
      }
      
      // Issues
      if (healthAnalysis.issues.length > 0) {
        yPos = addNewPageIfNeeded(yPos, 40);
        yPos = addCard('AREAS FOR IMPROVEMENT', yPos, Math.max(30, healthAnalysis.issues.length * 8 + 15));
        
        healthAnalysis.issues.forEach((issue, index) => {
          const lines = wrapText(`⚠ ${issue}`, contentWidth - 20, 9);
          doc.setFontSize(9);
          doc.setFont('helvetica', 'normal');
        ...
        yPos += Math.max(35, healthAnalysis.issues.length * 8 + 20);
      }
      
      // Recommendations
      if (healthAnalysis.recommendations.length > 0) {
        yPos = addNewPageIfNeeded(yPos, 40);
        yPos = addCard('STRATEGIC RECOMMENDATIONS', yPos, Math.max(30, healthAnalysis.recommendations.length * 8 + 15));
        
        healthAnalysis.recommendations.forEach((rec, index) => {
          const lines = wrapText(`${index + 1}. ${rec}`, contentWidth - 20, 9);
          doc.setFontSize(9);
          doc.setFont('helvetica', 'normal');
          doc.setTextColor(0, 0, 0);
          lines.forEach((line, lineIndex) => {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.setFont('helvetica', 'normal');
        doc.text('Generated by Snarbles Tokenomics Designer', margin, pageHeight - 12);
        doc.text('Professional Token Economy Analysis • snarbles.xyz', margin, pageHeight - 6);
      ...
      toast({
        ...
        description: `Your professional tokenomics report has been downloaded as ${fileName}`,
      });
    } catch (error) {
        ...
        <Card className="glass-card border-0 bg-background/50 backdrop-blur">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <Button 
              ...
              variant="outline" 
              className="border-border hover:bg-muted h-12 sm:h-auto relative"
            >
              {isGeneratingPDF ? (
                <>
            ...
            <Button 
              onClick={() => {
                toast({
                  ...
                  description: "Your report will include: Executive Summary, Distribution Analysis, Health Assessment, Market Scenarios, and Strategic Recommendations with professional formatting.",
                  duration: 5000,
                });
              }}
              variant="outline"
