...
export async function buildAtomicTokenCreationGroup(
  ...
  const feeConfig = getPlatformFeeConfig(networkType);
...
export async function signAndSubmitAtomicGroup(
  ...
  try {
    if (onStepUpdate) {
      onStepUpdate('wallet-approval', 'in-progress', { message: 'Please approve both transactions in your wallet' });
    }
    
    // Sign both transactions
    console.log('📝 Signing fee payment transaction...');
    ...
    if (onStepUpdate) {
      onStepUpdate('wallet-approval', 'completed', { message: 'Both transactions signed successfully' });
    ...
    const groupResponse = await algodClient.sendRawTransaction([signedFeeTransaction, signedTokenTransaction]).do();
...
export async function validateUserBalance(
  ...
  try {
    ...
    const balanceCheck = hasSufficientBalance(balance, network);
...
export async function createAlgorandToken(
  ...
  try {
    ...
    if (feeConfig.enabled) {
      ...
      const feeTrackingRecord = createFeeTrackingRecord(
        atomicGroup.groupId,
        atomicGroup.feeAmount,
        network,
        creatorAddress,
        'pending'
      );
      ...
      try {
        await trackFeeCollection(feeTrackingRecord);
      ...
      if (!submissionResult.success) {
        // Update fee status to failed if available
        if (atomicGroup.groupId) {
          try {
            await updateFeeStatus(atomicGroup.groupId, 'failed');
          } catch (updateError) {
            console.warn('Failed to update fee status to failed:', updateError);
          }
        }
    ...
    if (!assetId) {
      ...
      // Update fee status to confirmed even without asset ID
      if (feeConfig.enabled && groupId) {
        try {
          await updateFeeStatus(groupId, 'confirmed', {
            fee_transaction_id: feeTransactionId,
            token_transaction_id: txId
          });
          console.log('✅ Fee status updated to confirmed (no asset ID)');
        } catch (updateError) {
          console.warn('⚠️ Failed to update fee status to confirmed:', updateError);
        }
      }
    ...
    // Update fee status to confirmed if fees were paid
    if (feeConfig.enabled && groupId) {
      try {
        await updateFeeStatus(groupId, 'confirmed', {
          fee_transaction_id: feeTransactionId,
          token_transaction_id: txId,
          asset_id: typeof assetId === 'bigint' ? Number(assetId) : assetId
        });
        console.log('✅ Fee status updated to confirmed');
      } catch (updateError) {
        console.warn('⚠️ Failed to update fee status to confirmed:', updateError);
