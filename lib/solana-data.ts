...
export const SOLANA_NETWORKS = {
  MAINNET: {
    ...
    programId: null, // Mainnet program not deployed yet
  ...
  TESTNET: {
    ...
    programId: '9oidC5V2vgYrDmnFWeatttrmageCjyS78Mv5VdpVXUp',
  ...
  DEVNET: {
    ...
    programId: '9oidC5V2vgYrDmnFWeatttrmageCjyS78Mv5VdpVXUp', // Will be updated when you provide the actual program ID
...
// Platform PDA (same for both testnet and devnet since same program ID)
export const PLATFORM_PDA = new PublicKey('BdQj6ASFSQ6Ymokb9gAJ1DBsHQS3VWSxFP3kJtLTo1Rj');

// Upgrade Authority
export const UPGRADE_AUTHORITY = new PublicKey('352YpA1YVHmN9Jirf5cDZdELWvsrP3DJVL7svAHJtmUj');
...
export const NETWORK_STATUS = {
  TESTNET: {
    status: 'LIVE',
    description: 'Ready for token creation',
    icon: '✅',
    message: 'Testnet ready for token creation'
  },
  DEVNET: {
    status: 'LIVE', 
    description: 'Ready for token creation',
    icon: '✅',
    message: 'Devnet ready for token creation'
  },
  MAINNET: {
    status: 'PLANNED',
    description: 'Deployment planned',
    icon: '📋',
    message: 'Mainnet deployment planned'
  }
} as const;
...
export const SOLANA_ERROR_CODES = {
  ...
  6001: 'TokenNotPaused',
  6002: 'Unauthorized',
  6003: 'InvalidTokenData',
  6004: 'TokenAlreadyInitialized',
  6005: 'InvalidCreationFee',
  6006: 'InsufficientFunds',
  6007: 'TokenTransferDisabled',
  6008: 'InvalidAmount',
  6009: 'MintDisabled',
  6010: 'BurnDisabled',
  6011: 'InvalidOwner',
  6012: 'AccountAlreadyInitialized',
  6013: 'InvalidInstruction',
  6014: 'ProgramPaused',
  6015: 'InvalidProgramState',
  6016: 'TokenNameTooLong',
} as const;
...
export function getSolanaConnection(network = CURRENT_SOLANA_NETWORK): Connection {
  return new Connection(network.rpcUrl, 'confirmed');
}

// Helper function to derive Platform State PDA
export function getPlatformStatePDA(programId = PROGRAM_ID): [PublicKey, number] {
  return PublicKey.findProgramAddressSync(
    [Buffer.from(PDA_SEEDS.PLATFORM_STATE)],
    programId
  );
}

// Helper function to derive Token Data PDA
export function getTokenDataPDA(
  mintPublicKey: PublicKey,
  programId = PROGRAM_ID
): [PublicKey, number] {
  return PublicKey.findProgramAddressSync(
    [Buffer.from(PDA_SEEDS.TOKEN_DATA), mintPublicKey.toBuffer()],
    programId
  );
}

// Helper function to derive User State PDA
export function getUserStatePDA(
  userPublicKey: PublicKey,
  programId = PROGRAM_ID
): [PublicKey, number] {
  return PublicKey.findProgramAddressSync(
    [Buffer.from(PDA_SEEDS.USER_STATE), userPublicKey.toBuffer()],
    programId
  );
}
...
export function validateTokenName(name: string): boolean {
  return name.length > 0 && name.length <= PLATFORM_CONFIG.MAX_NAME_LENGTH;
}

export function validateTokenSymbol(symbol: string): boolean {
  return symbol.length > 0 && symbol.length <= PLATFORM_CONFIG.MAX_SYMBOL_LENGTH;
}

export function validateTokenDescription(description: string): boolean {
  return description.length <= PLATFORM_CONFIG.MAX_DESCRIPTION_LENGTH;
}

export function validateTokenUrl(url: string): boolean {
  return url.length <= PLATFORM_CONFIG.MAX_URL_LENGTH;
}

export function validateDecimals(decimals: number): boolean {
  return decimals >= 0 && decimals <= PLATFORM_CONFIG.MAX_DECIMALS;
}

export function validateCreationFee(feeInSol: number): boolean {
  return feeInSol >= 0 && feeInSol <= PLATFORM_CONFIG.MAX_CREATION_FEE_SOL;
}

// Network switching helper
export function getNetworkByName(networkName: string) {
  return Object.values(SOLANA_NETWORKS).find(network => 
    network.name.toLowerCase() === networkName.toLowerCase()
  );
}

// Explorer URL helpers
export function getTransactionUrl(signature: string, network = CURRENT_SOLANA_NETWORK): string {
  const clusterParam = network.isTestnet ? `?cluster=${network.name.toLowerCase()}` : '';
  return `${network.explorerUrl}/tx/${signature}${clusterParam}`;
}

export function getAccountUrl(address: string, network = CURRENT_SOLANA_NETWORK): string {
  const clusterParam = network.isTestnet ? `?cluster=${network.name.toLowerCase()}` : '';
  return `${network.explorerUrl}/account/${address}${clusterParam}`;
}

export function getProgramUrl(programId: string, network = CURRENT_SOLANA_NETWORK): string {
  const clusterParam = network.isTestnet ? `?cluster=${network.name.toLowerCase()}` : '';
  return `${network.explorerUrl}/account/${programId}${clusterParam}`;
}
