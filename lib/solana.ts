...
export async function createTokenOnChain(
  ...
  try {
    ...
    // Use alternative direct token creation method to bypass the buggy contract
    if (onStepUpdate) {
      onStepUpdate('method-selection', 'in-progress', { message: 'Using direct Solana token creation...' });
    }
    
    const { createSolanaTokenDirect } = await import('./solana-alternative');
    return await createSolanaTokenDirect(wallet, tokenData, options);
  } catch (error) {
